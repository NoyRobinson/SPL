...
class Dao(object):
    ...
    def update(self, dto_instance, **keyvals):
        update_dict = vars(dto_instance)
        params = list(keyvals.values()) + list(update_dict.values())[0:1]
        
        stmt = 'UPDATE {} SET {} WHERE {}' \
            .format(self._table_name, ', '.join([col + '=?' for col in keyvals.keys()]), ' And '.join([col + '=?' for col in list(update_dict.keys())[0:1]]))
